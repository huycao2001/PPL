...
// For Static access , It will be in the form ID :: Dollar_ID
//--------------------- Parser-------------------------//
...
testing2 :  att_name (COMMA att_name )* COLON compound_type ; // Var a,b,c :  Int  ;
...
instance_call_stmt : (expression7 DOT ID LB expressions_list? RB SEMI_COLON);

static_call_stmt : expression7 DOUBLE_COLON DOLLAR_ID LB expressions_list? RB SEMI_COLON;
// a[3] , a = b = 4 
...
expression7 : expression7 (LS expression RS)+ | expression8; // (a[i]).a
...
fragment INT_PART :  '0' | ([1-9][0-9]* ('_'[0-9]+)*) ;
